...
import {
  ...
  type RemoveTransactionActionState,
} from "@/modules/transactions/presentation/actions"

const initialState: RemoveTransactionActionState = {
  status: "idle",
}
...
function getScopeContent({
  ...
  if (installmentTotal) {
    return {
      ...
      description: supportsFutureRemoval
        ...
        : "Este lançamento é parcelado, mas só é possível remover a parcela selecionada porque a série não possui vínculo completo.",
      singleLabel: "Apenas esta parcela",
      futureLabel: "Esta e próximas parcelas",
      futureDescription: "Remove a parcela atual e todas as parcelas futuras da mesma série.",
    }
  }

  if (isFixed) {
    return {
      title: "Remover lançamento fixo",
      description: supportsFutureRemoval
        ? "Escolha se deseja remover apenas este lançamento ou este e os próximos lançamentos fixos."
        : "Este lançamento é fixo, mas só é possível remover o item selecionado porque a série não possui vínculo completo.",
      singleLabel: "Apenas este lançamento",
      futureLabel: "Este e próximos lançamentos",
      futureDescription: "Remove o lançamento atual e todos os futuros da mesma série fixa.",
    }
  }
  ...
  return {
    ...
    description:
      "O lançamento será removido. Se ele já estiver compensado, o saldo da conta será ajustado automaticamente.",
    singleLabel: "Apenas este lançamento",
    futureLabel: "",
    futureDescription: "",
  }
...
export function TransactionRemoveButton({
  transactionId,
  transactionTitle,
  isFixed,
  installmentTotal,
  ...
  const isRecurring = isFixed || Boolean(installmentTotal)
  const content = getScopeContent({ isFixed, installmentTotal, supportsFutureRemoval })
      ...
      <DialogContent className="border border-white/10 bg-[#141414] p-0 text-white ring-0 sm:max-w-md">
        <DialogHeader className="border-b border-white/10 px-6 py-5">
          ...
          <DialogDescription className="text-white/60">
            <strong className="text-white">{transactionTitle}</strong>. {content.description}
          </DialogDescription>
        </DialogHeader>
              ...
              <FieldContent>
                ...
                {!supportsFutureRemoval ? (
                  <FieldDescription className="mt-2 text-white/45">
                    Séries novas já suportam remoção em lote dos lançamentos futuros.
                  </FieldDescription>
                ) : null}
              </FieldContent>
            </Field>
